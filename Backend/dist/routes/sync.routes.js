...
export async function syncRoutes(app) {
    app.post('/sync/favorites', { preHandler: [app.authenticate] }, async (request, reply) => {
        try {
    ...
    app.get('/sync/favorites', { preHandler: [app.authenticate] }, async (request, reply) => {
        try {
            const userId = request.user.userId;
            const stored = await syncRepository.getFavorites(userId);
            const result = stored.map(f => ({
    ...
    app.post('/sync/playlists', { preHandler: [app.authenticate] }, async (request, reply) => {
        try {
    ...
    app.get('/sync/playlists', { preHandler: [app.authenticate] }, async (request, reply) => {
        try {
            const userId = request.user.userId;
            const stored = await syncRepository.getPlaylists(userId);
            const result = stored.map(p => ({
